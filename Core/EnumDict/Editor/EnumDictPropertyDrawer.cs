...
namespace Utils.Core.EnumDict
{
    ...
    public class EnumDictPropertyDrawer : PropertyDrawer
    {
        ...
        private static int GetEntryCount(SerializedProperty dict) => dict.FindPropertyRelative("entries").arraySize;

        private static SerializedProperty GetEntry(SerializedProperty dict, int index) => dict.FindPropertyRelative("entries").GetArrayElementAtIndex(index);

        private static IEnumerable<SerializedProperty> GetEntries(SerializedProperty dict)
        {
            var count = GetEntryCount(dict);
            return Enumerable.Range(0, count).Select(i => GetEntry(dict, i));
        }

        private static Type GetEnumType(SerializedProperty dict) => Type.GetType(dict.FindPropertyRelative("enumTypeName").stringValue);

        private static string GetName(SerializedProperty entry, Type enumType) => Enum.GetName(enumType, entry.FindPropertyRelative("enum").intValue);
        ...
        public override void OnGUI(Rect position, SerializedProperty dict, GUIContent label)
        {
            ...
            Rect RelativeRect(float dx, float dy, float w, float h) => new Rect(position.x + dx, position.y + dy, w, h);
